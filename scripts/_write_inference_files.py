...
_DEFAULT_DISEASE_MAPPING = Path("data/processed/disease_mapping.json")
...
class NLPInferencePipeline:
    ...
    def __init__(
        ...
        Args:
            ...
            disease_mapping_path: Path to disease_mapping.json (disease to index).
        ...
        # The checkpoint was saved from SymptomClassifier which wraps
        # DistilBertForSequenceClassification under self.model.
        # Keys are prefixed with "model." e.g. "model.distilbert.*".
        # Strip prefix to load directly into DistilBertForSequenceClassification.
        saved_state: Dict[str, Any] = raw["model_state_dict"]

        sample_key = next(iter(saved_state))
        if sample_key.startswith("model."):
            logger.info(
                "Stripping 'model.' key prefix from checkpoint state dict "
                "(SymptomClassifier -> DistilBertForSequenceClassification)."
            )
        ...
        logger.info(
            "NLP checkpoint loaded - epoch %s | val_loss %s | val_acc %s",
    ...
    def _load_disease_mapping(
        ...
        """Loads disease_mapping.json and builds reverse index-to-name map."""
...
pathlib.Path("src/inference/nlp_predict.py").write_text(NLP_PREDICT, encoding="utf-8")
print("src/inference/nlp_predict.py written OK")
