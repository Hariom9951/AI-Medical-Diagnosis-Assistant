...
class S3StorageConfig(BaseModel):
    ...
    endpoint_url: str | None = Field(None, description="Optional custom endpoint URL for local MinIO/mock servers")
...
class ModelConfig(BaseModel):
    ...
    vision_model_name: str = Field("vit_base_patch16_224", description="Vision backbone transformer architecture key")
    vision_img_size: int = Field(224, gt=0, description="Square input resolution dimension (e.g. 224x224)")
    text_model_name: str = Field("emilyalsentzer/Bio_ClinicalBERT", description="Clinical text processing encoder name")
    embedding_dim: int = Field(768, gt=0, description="Embedding vector size output by text encoder")
    fusion_hidden_dim: int = Field(256, gt=0, description="Dense projection dimension size for fused modalities")
    num_classes: int = Field(5, gt=0, description="Number of target diagnosis categories")
    target_gradcam_layer: str = Field("blocks.11.norm1", description="Target backbone layer for visual explainability extraction")
...
class TrainingConfig(BaseModel):
    ...
    learning_rate: float = Field(1e-4, gt=0.0, lt=1.0, description="Learning optimizer speed constant")
    checkpoint_dir: Path = Field(..., description="Local directory target to save training weight runs")
